...
def generateHTMLReport(testResult):
    ...
    report_html.write("<!--\n")
    #report_html.write("@font-face{font-family:\"Segoe UI\";font-weight:400;src:local(\"Segoe UI\")}\n")
    #report_html.write("@font-face{font-family:\"Segoe UI\";font-weight:700;src:local(\"Segoe UI Bold\")}\n")
    #report_html.write("body{margin:0; font-family:Tahoma;Simsun;Segoe UI;font-size:12px;}\n")
...
#blue color: #034579
    ...
    report_html.write("<body><table cellpadding=\"0\" cellspacing=\"0\" align=\"center\"\n>")
    report_html.write(
        "<tr align=center class=title><td width=\"60\">Run Time</td><td width=\"150\">Test Suite</td><td width=\"60\">Exec ID</td><td width=\"200\">TestCaseName</td><td width=\"80\">Result</td><td width=\"500\">FailedInfo</td></tr>\n")
    ...
    for testsuite in testResult.testsuite_list:
        ...
        for item in testcaseList:
            ...
            if status == "Passed":
                report_html.write("<tr class=desc><td>" \
                                  + str(testResult.testsuite_list.index(testsuite)+1) + "</td>" + "<td>" \
                                  + testsuite.param.testname + "</td><td><B>" \
                                  + str(exec_id) + "</B></td><td>" \
                                  + testcase_name + "</td><td style=\"color:green;\">" \
                                  + status+"</td><td>"+"None"+"</td></tr>\n")
            else:
